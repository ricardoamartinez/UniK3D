    ...
    if 'depth' in predictions:
        ...
        if edge_params_ref.get('planar_projection', False):
            ...
            if current_depth_map is not None:
                if current_depth_map.ndim == 2:
                    H_orig, W_orig = current_depth_map.shape
                elif current_depth_map.ndim == 1:
                    # Handle case where depth map is flattened - try to infer dimensions
                    total_pixels = current_depth_map.shape[0]
                    ...
                    common_resolutions = [
                        ...
                        (720, 1280),   # HD
                        (1080, 1920),  # Full HD
                        (384, 512),    # Common model output
                        (192, 256),    # Smaller model output
                    ]
                    ...
                    for h, w in common_resolutions:
                        if total_pixels == h * w:
                            H_orig, W_orig = h, w
                            current_depth_map = current_depth_map.reshape(H_orig, W_orig)
                            found_match = True
                            break
                    
                    if not found_match:
                        # Try to infer square-ish dimensions for unusual sizes
                        sqrt_pixels = int(total_pixels ** 0.5)
                        if sqrt_pixels * sqrt_pixels == total_pixels:
                            H_orig, W_orig = sqrt_pixels, sqrt_pixels
                            current_depth_map = current_depth_map.reshape(H_orig, W_orig)
                            print(f"Info: Inferred square depth map dimensions {H_orig}x{W_orig} from {total_pixels} pixels")
                            found_match = True
                        else:
                            # Try common aspect ratios
                            for ratio_h, ratio_w in [(3, 4), (9, 16), (1, 1)]:
                                for scale in range(1, 100):
                                    h, w = ratio_h * scale, ratio_w * scale
                                    if h * w == total_pixels:
                                        H_orig, W_orig = h, w
                                        current_depth_map = current_depth_map.reshape(H_orig, W_orig)
                                        print(f"Info: Inferred depth map dimensions {H_orig}x{W_orig} from {total_pixels} pixels")
                                        found_match = True
                                        break
                                if found_match:
                                    break
                    ...
                    if not found_match:
                        print(f"Warning: Cannot infer depth map dimensions from 1D tensor with {total_pixels} pixels")
                        print(f"Info: Falling back to spherical projection instead of planar")
                        H_orig, W_orig = (480, 640)
                        current_depth_map = None
                        # Disable planar projection for this frame
                        edge_params_ref['planar_projection'] = False
                else:
                    print(f"Warning: Unexpected depth map dimensions: {current_depth_map.shape}")
                    H_orig, W_orig = (480, 640)
                    current_depth_map = None

            if current_depth_map is not None and current_depth_map.ndim == 2:
                H, W = current_depth_map.shape
            else:
                H, W = H_orig, W_orig
            
            # Only proceed with planar projection if we have a valid depth map
            if current_depth_map is not None:
                input_fov_deg = edge_params_ref.get('input_camera_fov', 60.0)
                ...
                print("Warning: Planar projection requested but no valid depth map available.")
                points_xyz = None
            ...
            if current_depth_map is not None:
                if current_depth_map.ndim == 2:
                    ...
                    for h, w in common_resolutions:
                        if total_pixels == h * w:
                            H_sph, W_sph = h, w
                            current_depth_map = current_depth_map.reshape(H_sph, W_sph)
                            print(f"DIAGNOSTIC: Spherical mode - Reshaped 1D depth map ({total_pixels} pixels) and overriding with constant {forced_depth_value_spherical}")
                            found_match = True
                            break
                    ...
                    if not found_match:
                        ...
                        sqrt_pixels = int(total_pixels ** 0.5)
                        if sqrt_pixels * sqrt_pixels == total_pixels:
                            H_sph, W_sph = sqrt_pixels, sqrt_pixels
                            current_depth_map = current_depth_map.reshape(H_sph, W_sph)
                            print(f"Info: Spherical mode - Inferred square depth map {H_sph}x{W_sph} from {total_pixels} pixels")
                            found_match = True
                        else:
                            # Try common aspect ratios
                            for ratio_h, ratio_w in [(3, 4), (9, 16), (1, 1)]:
                                for scale in range(1, 100):
                                    ...
                                    if h * w == total_pixels:
                                        H_sph, W_sph = h, w
                                        current_depth_map = current_depth_map.reshape(H_sph, W_sph)
                                        print(f"Info: Spherical mode - Inferred depth map {H_sph}x{W_sph} from {total_pixels} pixels")
                                        found_match = True
                                        break
                                if found_match:
                                    break
                    ...
                    if not found_match:
                        print(f"Warning: Cannot infer spherical depth map dimensions from 1D tensor with {total_pixels} pixels")
                        H_sph, W_sph = (480, 640)
                        current_depth_map = None
                else:
                    print(f"Warning: Unexpected spherical depth map dimensions: {current_depth_map.shape}")
                    H_sph, W_sph = (480, 640)
                    current_depth_map = None
